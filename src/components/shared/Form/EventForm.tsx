...
export default function CreateEventForm({
  ...
  const initialValues =
    event && type === "Update"
      ? {
          ...event.event,
          startDateTime: new Date(event.event.startDateTime),
          endDateTime: new Date(event.event.endDateTime),
          // organizer: userId,
          category: event.event.category._id,
        }
      : eventDefaultValues;

  type EventFormValues = z.infer<typeof eventFormSchema> & { _id?: string };
  ...
  const createEventMutation = useMutation({
    ...
    onError: (error) => {
      // toast.error("Network delay detected. Please try again.");
    },
  });
